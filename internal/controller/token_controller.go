...
func (r *TokenReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	if managedSecretName != "" && managedSecretName != secretName {
		oldSecret := &corev1.Secret{}
		err = r.Get(ctx, types.NamespacedName{Name: managedSecretName, Namespace: secretNamespace}, oldSecret)
		if err == nil && metav1.IsControlledBy(oldSecret, token) {
			log.Info("deleting old secret", "namespace", secretNamespace, "secret", managedSecretName)
	...
	case err == nil:
		if !metav1.IsControlledBy(secret, token) {
			err = fmt.Errorf("secret not owned by token")
			log.Error(err, "ownership mismatch", "token", token.GetName(), "secret", secretName, "namespace", secretNamespace)

			token.SetStatusCondition(metav1.Condition{
				Type:    conditionTypeAvailable,
				Status:  metav1.ConditionFalse,
				Reason:  "Failed",
				Message: fmt.Sprintf("Secret not owned by token (%s): (%s)", token.Name, err),
			})
